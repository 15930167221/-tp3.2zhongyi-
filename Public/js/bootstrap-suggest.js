...
(function (factory) {
    ...
    function getPointKeyword($list) {
		//miracle7kill 注释掉控制上下键移动同时input改变
        //return $list.data();
    }
	//miracle7kill add
	function getEnterPointKeyword($list)
	{
    ...
    var defaultOptions = {
        ...
        keyField: 'other_name',                   // 每组数据的哪个字段作为输入框内容，优先级高于 indexKey 设置（推荐）
            ...
            return self.each(function() {
                ...
                $input.on('keydown', function(event) {
                    ...
                    if (event.keyCode === options.keyDown) { // 如果按的是向下方向键
						...
						//miracle7kill回车后再改变input的值
						tipsKeyword = getEnterPointKeyword(currentList);
                ...
                $dropdownMenu.mouseenter(function() {
                    ...
                    .on('mousedown', 'tbody tr', function() {
						...
						//miracle7kill支持点击改变input的值
						setValue($input, getEnterPointKeyword($(this)), options);
                        setBackground($input, options);
                        $dropdownMenu.hide();
                    });
