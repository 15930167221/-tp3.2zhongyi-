...
class IndexController extends PublicController  {
    ...
    public function home($rev){
        $res=M()->field('b1.id as id1,b1.department,b2.id as id2,b2.levelid,b2.pid')
        ->table("user_info_dict as b1,about as b2")
...
//        $userInf=$user->query("select br_id,br_name,xh,xb,nl,convert(varchar(19),p_date,120) as p_date from station_p where br_id='".session('id')."'' and xh=".session('xh'));
    ...
    public function jiezhen(){
        ...
        //以上是分页 ， 以下是数据
        //'jz_flag=1'
        $data =  $rect->where($where)->order('p_date desc')->limit($page->firstRow.','.$page->listRows)->field('convert(varchar(19),p_date,120) as p_date,convert(varchar(19),jz_date,120) as jz_date,convert(varchar(19),cs_date,120) as cs_date,br_name,xb, dbo.get_age(convert(varchar(19),cs_date,120),convert(varchar(19),jz_date,120)) as nl,nl as nlt,tel,br_id,xh')->select();//查询数据（未完成就诊的）$Page->firstRow 起始条数 $Page->listRows 获取多少条
        // dump($data);die;
//         echo $rect->getLastSql();
        ...
        $djxh = I('get.djxh');
        if (!empty($djid)) {
            $this->assign('dataid',$djid);// 赋值模板变量
            $this->assign('dataxh',$djxh);// 赋值模板变量
        }
    ...
    public function dojiezhenajaxxiugai(){
        ...
        $where['br_id'] = $id;
        $where['xh'] = $xh;
        $data->where($where)->save($user);//执行修改
        $this->redirect("Index/jiezhen");//重定向
    }
    //点击存session  接诊区
    public function saveSession(array $info){
    ...
    public function dengji(){
        ...
        if (!empty($getid)) {
            $user = M('station_p');// 实例化Data数据模型
            $data = $user->where("br_id='{$getid}' and xh='{$xh}'")->field('convert(varchar(19),p_date,120) as p_date,br_name,xb,nl,tel,br_id,xh,dw,pass,e_mail,ghf,fax,convert(varchar(10),cs_date,120) as cs_date')->select();
            // dump($data);die;
    ...
    public function hzbc(){
        ...
        if(empty($br_name)){
            ...
            $this->redirect('Index/jiezhen', array("djid" => $br_id ,"djxh" => $dangqingjiuzhengcishu));//重定向到接诊区   
        }
    ...
    public function ajaxtongming(){
        ...
        $where['br_name'] = $name;
         //获取所属机构
            $dpment = session('dpment');
            $where['department'] = $dpment;
        $data = $user->where($where)->field('convert(varchar(19),p_date,120) as p_date,convert(varchar(19),jz_date,120) as jz_date,convert(varchar(10),cs_date,120) as cs_date,br_name,xb, dbo.get_age(convert(varchar(19),cs_date,120),convert(varchar(19),jz_date,120)) as nl,tel,br_id,xh,pass,dw,fax,e_mail')->select();
    ...
    public function yuyue(){
        ...
        $user = M('station_p');// 实例化Data数据模型
        if (!empty($getid)) {
            $datazuo = $user->where("br_id={$getid} and xh={$xh}")->field('convert(varchar(19),p_date,120) as p_date,br_name,xb,nl,tel,br_id,xh,dw,pass,e_mail,ghf,fax,convert(varchar(10),cs_date,120) as cs_date')->select();
    ...
    public function chaxun(){
        ...
        if ($qufenbiaodan == 1) {
            ...
            $p_datekai = I('get.p_datekai');//获取开始日期
            $p_datezhong = I('get.p_datezhong');//获取终止日期
            // dump($p_datezhong);
            ...
            $data =  $rect->where($com)->order('p_date desc')->limit($page->firstRow.','.$page->listRows)->field('convert(varchar(19),p_date,120) as p_date,br_name,xb,nl,tel,br_id,xh,dw,pass,e_mail,ghf,fax,convert(varchar(19),cs_date,120) as cs_date')->select();
    ...
    public function chaxunxiangxi(){
        $id = I('get.id');
        $user = M('station_p');
        $data = $user->where("br_id={$id}")->field('convert(varchar(19),p_date,120) as p_date,br_name,xb,dbo.get_age(convert(varchar(19),cs_date,120),convert(varchar(19),jz_date,120)) as nl,tel,br_id,xh,dw,pass,e_mail,ghf,fax,convert(varchar(10),cs_date,120) as cs_date')->select();
        $this->ajaxReturn($data,'json');
    }
    //健康档案
...
//        echo session('id');
        //接受从患者登记处传值（post方式）
        $station = D('station_p');//链接数据库
        if (IS_POST){
            $br_name = I('post.br_name');//姓名
            $xb = I('post.xb');//性别
            $ghf = I('post.ghf');//挂号费
            //判断挂号费
            if(empty($br_name)){
...
//        echo $xh;
        ...
        if($blh && $xh){
            ...
            //判断是否有保存数据
            $jkda=D('jkda_xx');//实例化健康档案选项信息表
            $bqxx=D('bqxx');
            //如果通过历史就诊记录传值
            if(I('get.jid') && I('get.jxh')){
                $blh=I('get.jid');
                $xh=I('get.jxh');
                $this->assign('lijzjl',1);   //判断历史就诊记录弹框
                $this->assign('lijzxh',$xh); //历史就诊记录序号
            }
            //查询条件
            $whereConf['br_id']=$blh;
            $whereConf['xh']=$xh;
...
//            dump($xyChuzhi);
            ...
            $nameRes=substr($xyNameXYB,0,-1);//去掉逗号后的西药名称
            if($xyChuzhi){
                
                $this->assign('xyflag',1);//经过西药开方
...
//        $wcjz=I("get.wcjz"); //完成就诊判断，1点击保存，2点击完成就诊
        ...
        $userJKInf['userids']=session(wh_userId);//就诊医生id
        $data = array();
        ...
        $data1['XH']=session(xh);
        $data1['userids']=session(wh_userId);//就诊医生id
...
//        $data1['jz_date']=date("Y-m-d H:i:s");
        ...
        if($data['br_id'] && $data['xh']){
            ...
            if($jkdaConf || $bqxxConf){
                ...
                $jkdaChange=$jkda->where($data)->save($jkdaInf);  //更新到jkda_xx表
                $bqxxChange=$bqxx->where($whereConf)->save($data1);  //更新到bqxx表
...
//    体质辨识答题界面
    public function tizhi(){
        ...
        if($blh && $xh){
            ...
            $userInf=D('tz_jbxx')->getUserAns($userWhere);//患者的答题记录表,患者的选项信息
            $data=D('tz_jieguo')->getUserInfAndAnsJG($userWhere);//患者的个人信息和答题结果
//            判断患者是否存在保存答题信息
            if($userInf && $data){
                $this->assign('dati',1);//不需要判断答题数目
                if($tzbs){
...
//              print_r($tzbs);
    ...
    public function tizhiSub(){
        ...
        $where['xh']=$xh;
        $where['br_id']=$blh;
        $userInf=D('station_p')->getUserInf($where);//根据序号查出患者信息
...
//        $data['date'] = substr($blh,0,4)."-".substr($blh,4,2)."-".substr($blh,6,2);
        ...
        $_POST['userids']=session('wh_userId');//就诊医生ID

        //体质辨识算法结束
        session('userXXInf',$_POST);//保存患者选项信息
        //$data患者的相关信息以及对应的九种体制信息
        $res1=array_slice($data,9,27);
        $res1['userids']=session('wh_userId');//就诊医生ID
...
//          必须登记后才能进行答题
    ...
    public function tizhiSave(){
        ...
        $userXXInf=session(userXXInf);//患者选项信息包含就诊医生id
...
//      数组组合 $data患者病历号和序号 $res1患者的
        ...
        //根据序号和病历号查询病人的相关记录
        $userInf1=$jbxx->getUserAnsInf($where);  //存放患者选项信息的表
        $jieguoInf1=$jieguo->getUserAnsJG($where);  //存放答题结果信息表
