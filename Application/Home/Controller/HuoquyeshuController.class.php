...
class HuoquyeshuController extends Controller
{
    public function dangri(){
        ...
        $sql = "select t.rn from (select *,row_number() over(order by p_date DESC) rn from STATION_P WHERE department = '$dpment' AND jz_flag = '1' AND p_date BETWEEN '$tjri 00:00:00' AND '$tjri 23:59:59') t where t.br_id='$blh' AND t.xh='$sblhxh'";
        $data = $user->query($sql);
        $b = 9;
        $yeshu = $data['0']['rn']/$b;
    ...
    public function shangzhou(){
        ...
        //获取所属机构
        $dpment = session('dpment');
        $sql = "select t.rn from (select *,row_number() over(order by p_date DESC) rn from STATION_P WHERE department = '$dpment' AND jz_flag = '1' AND p_date BETWEEN '$dotjri 00:00:00' AND '$tjri 23:59:59') t where t.br_id='$blh' AND t.xh='$sblhxh'";
        $data = $user->query($sql);
        $b = 9;
        $yeshu = $data['0']['rn']/$b;
        $quzhengyeshu = ceil($yeshu);
