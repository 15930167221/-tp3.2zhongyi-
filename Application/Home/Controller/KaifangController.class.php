...
class KaifangController extends PublicController {
    ...
    public function bingMing(){
        $zhengxinghanzi = M('y_semiotics');
            $zxhanziwhere['TREE'] = '10';
            $zxhanzidata = $zhengxinghanzi->where($zxhanziwhere)->field("CONVERT(VARCHAR(MAX),DECRYPTBYKEY(NAME)) as NAME")->select();
        $where['BM'] = '000';
        $data = M('tcd_zybm')->where($where)->Distinct(true)->
...
WHERE bz_cf.cfdm = '$tjyouzhengxing' and dict_drug_zy.department = '$dpment' order by serial_no";
    ...
    function fangjie(){ 
        ...
        /*$result=$model->field('bz_cf.*,b2.drug_name')
                    ...
                    ->where("bz_cf.cfdm='$_POST[tree]' and department=".session('dpment'))
                    ->order('serial_no')
                    ->select();
    ...
    public function bianZheng(){
        ...
        $where['ISSHOW']= 0;
        $data =  M('y_mainsypmpotm')->where($where)->Distinct(true)->
        ...
        // $maixiangjsdata = $jzuser->getBmInfoName($maixiangjswhere);
        // $this->assign("maixiangjsdata",$maixiangjsdata);

        // //获取脉象的常用选择
        // $maixiangcyxxwhere['MNEMONIC'] = 1;
        // $maixiangcyxxwhere['KIND'] = 3;
        // $maixiangcyxxdata = $jzuser->getBmInfoName($maixiangcyxxwhere);
        // $this->assign("maixiangcyxxdata",$maixiangcyxxdata);
        $this->display();
    }
    //  ------------------------------------------主症开始----------------------------
    //页面6 ajax 点击主症
    ...
    public function zhuzxcodezhuanhz(){
        $dozhuzheng = I('post.dozhuzheng');//接受传过来的输入的值
        $pieces = explode( " ", $dozhuzheng);//转为数组
        $user = D("y_mainsypmpotm");
        $where['TREE'] = array("in",$pieces);
        $where['ISSHOW']= 1;
        $data = $user->getBmInfoName($where);
    ...
    public function yeliuzhuzhengjssousuokuang(){
        $user = D("y_mainsypmpotm");
        $tjzzjs = I('post.tjzzjs');//接受传过来的输入的值
        // ISSHOW = 1 为主症下的症状
        $where['ISSHOW'] = 1;
        //判断是否为空
        if (!empty($tjzzjs)) {
    ...
    public function yeliuAjaxjzss(){
        $zxzfjg = I('post.zxzfjg');//接受传过来的输入的值
        $zhuzhengTree = I('post.zhuzhengTree');//接受传过来的主症tree号
        $dozhuzhengTree =trim($zhuzhengTree);//去除最后的空格
        $user = D("y_concomitancesypmptom");
        //模糊匹配符合主症的tree
    ...
    public function yeliuAjaxJZFensheweicyxx(){
        $jzfenlsheweicycode = I('post.jzfenlsheweicycode');//接受传过来的输入的值
    ...
    public function yeliuAjaxJZjssheweicyxx(){
        $jzjssheweicycode = I('post.jzjssheweicycode');//接受传过来的输入的值
        $user = M("y_concomitancesypmptom");
    ...
    public function yeliuAjaxjzjssousuofuzhi(){
        $user = D("y_concomitancesypmptom");
        $zxzfjg = I('post.zxzfjg');//接受传过来的输入的值
        $zhcztj = I('post.zhcztj');//接受传过来的主症号
         $dozhuzhengTree =trim($zhcztj);//去除最后的空格
        $where['TREE'] = array('like',"{$dozhuzhengTree}%");
        $where['KIND'] = 1;
        //判断是否为空
        if (!empty($zxzfjg)) {
    ...
    public function jzajaxcysousuokuang(){
        $jzshurutj = I('post.jzshurutj');//接受传过来的输入的值
        $user = D("y_concomitancesypmptom");
        //判断是否为空
        if (!empty($jzshurutj)) {
            //判断是否是字母
    ...
    public function yeliuAjaxshexiangdishexiangchs(){
        $mod = M();
        $shexiangzhcztj = I('post.shexiangzhcztj');//接受传过来的输入的值
        $sxjioj = trim($shexiangzhcztj);//去除最后的空格
...
SELECT MIN(code) FROM [y_concomitancesypmptom] WHERE [TREE] LIKE '$sxjioj%' AND [KIND] = '2' group by tree) ";
    ...
    public function yeliuAjaxshexiangFensheweicyxx(){
        $shexiangfenlsheweicycode = I('post.shexiangfenlsheweicycode');//接受传过来的输入的值
        $user = M("y_concomitancesypmptom");
...
//页面6 ajax 脉象分类设为常用选择
    public function yeliuAjaxmaixiangFensheweicyxx(){
        $maixiangfenlsheweicycode = I('post.maixiangfenlsheweicycode');//接受传过来的输入的值
        $user = M("y_concomitancesypmptom");
    ...
    public function yeliuAjaxmaixiangjssheweicyxx(){
        $jzjsmaiweicycode = I('post.jzjsmaiweicycode');//接受传过来的输入的值
        $user = M("y_concomitancesypmptom");
    ...
    public function west(){
        ...
        $this->assign('xycfId',$xycfList);//西药处方号传递
        $xytms = count($xycfList)+1;//单个病人的西药处方数量加一
        //获取当日处方最大的ID
        $where['cf_id'] = array('like',"{$day}%");
        ...
        $this->assign("xytms",$xytms);//最新条目数
        $this->assign("maxId",$newId);//最新处方号传递
        $this->display();
    }
...
//        $cfna =array();
 ...
 $dictyz = M('dict_order');
        ...
        foreach($historyCF as $key=>$val){
            ...
            }else if($historyCF[$key]['indicate']==1){
                $historyCF[$key]['indicate']='已审';
            }else if($historyCF[$key]['indicate']==2){
...
$model=D('YMakenature');//性味归经
 ...
 $v['qev']=substr($v['tree'],0,12);
    ...
    function apds(){
        $model=M('drug_dict');
        $result=$model->where("drug_code='$_POST[code]'")->select();
        $this->ajaxReturn($result);
    }
    /**
     * 双击历史处方，保存成为当天处方
...
//        $cfna =array();
        ...
        $xh=session(xh);
        $chufang=M('Prescription');
        $historyCF=$chufang->join('station_p on Prescription.patient_id=station_p.br_id and Prescription.xh=station_p.xh')->field('presc_no,presc_name,convert(varchar(10),presc_date,120) presc_date,indicate,zy_name,bz,zz')->where("Prescription.patient_id='$blh' and station_p.jz_flag='2' and Prescription.xh<>'$xh'")->select();
        foreach($historyCF as $key=>$val){
            if($historyCF[$key]['indicate']==0){
                $historyCF[$key]['indicate']='<span style="color:red;">未审</span>';
            }else if($historyCF[$key]['indicate']==1){
