...
class AjaxController extends PublicController
{
    ...
    public function drugWest($htm)
    {
        ...
        if (preg_match("/^[a-z]/i", $htm)) {
            ...
            $list = $durg->where($where)->join('sys_dm_jldw on sys_dm_jldw.id = drug_dict.hl_unit')->field('drug_dict.drug_name,drug_dict.drug_code,drug_dict.input_code,drug_dict.drug_spec,drug_dict.price,drug_dict.hl,drug_dict.package_units,sys_dm_jldw.dw as units')->select();
            ...
            $dpment = session('dpment');
            $where2['drug_dict.department'] = $dpment;
            $where2['drug_dict.drug_name'] = array('like', "{$htm}%");
            $where2['drug_dict.drug_indicator'] = array('in', '01,03');
            $list = $durg->where($where2)->join('sys_dm_jldw on sys_dm_jldw.id = drug_dict.hl_unit')->field('drug_dict.drug_name,drug_dict.drug_code,drug_dict.input_code,drug_dict.drug_spec,drug_dict.price,drug_dict.hl,drug_dict.package_units,sys_dm_jldw.dw as units')->select();
        }
        $this->ajaxReturn($list);
    }
...
//        session('xyFlag',1);//判断健康档案，西医诊断的来源
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            $where['drug_code'] = (string) $abianma[$i + 1];
            $pr = $dic->where($where)->field('price')->select();
            $price = $pr[0][price];
            $list[$i]['yp_total_cost'] = $ashuliang[$i + 1] * $price;
            $list[$i]['price'] = $price;
            ...
            $list[$i]['department'] = session('dpment');
        }
        for ($j = 0; $j < $num; $j++) {
    ...
    public function westBing()
    {
        ...
        if (preg_match("/^[a-z]/i", $val)) {
            ...
            $list = $xy->where($where)->order('code desc')->select();
        } else {
    ...
    public function jia()
    {
        ...
        $name = $_POST['val'];
        $drug = M('drug_dict');
        $list = $drug->where("drug_code='{$name}'")->Field("drug_code")->find();
        $code = $name;
      $model = D('YMakenature');
      $xwyp = $model->getAll("code='$code'");
      $tree = $xwyp[0][tree];
      $arr = explode('.',$tree);
      $num = $arr[2];
      if($num == '001'){
        ...
        $code = I('post.val');
        $vDict = M('v_dict_drug_zy');
        $where['code'] = $code;
        $where['flag'] = 1;
        $list = $vDict->where($where)->field('code,xw')->find();
//        var_dump($list);die();
        $this->ajaxReturn($list);
    }
    //检查是不是孕妇
    public function women()
    {
        ...
        //超标
        session('cz', 1);
        //健康档案处置判断取值位置（1处方0.bqxx）
        $ypCodeList = I("post.val");
        $ypYlList = I("post.ypyl");
        $res = "";
        $jbxx = M('jbxx');
        //药品用量表
        $clyp = $jbxx->where("xxlbdm='03'")->select();
        //规定的超量药品数据
        $xxdm = i_array_column($clyp, 'xxdm');
        //二维数组转一维数组
        $comm = array_intersect($ypCodeList, $xxdm);
        //找出交集
        foreach ($comm as $key => $value) {
            $num = array_keys($ypCodeList, $value);
            ///$num为一个数组,确定交集中元素的位置
    ...
    public function getId()
    {
        ...
        if (count($_POST['cfcode']) > 1) {
          ...
          if($newId == 1){
            $newId = session('wh_userId').$day.'00001';
          }
          $this->ajaxReturn($newId);
    }
      //中医处方保存
      public function zyBaocun(){
        $zy_name = (string)$_POST['zy_name'];
        $zy_bz = (string)$_POST['zy_bz'];
        $zy_zz = (string)$_POST['zy_zz'];
        ...
        //查出价钱
        $dicPri = M('drug_dict');
        // $whpri['drug_code'] = array('in',$zycode);
        $whpri['department'] = session('dpment');
        for ($p = 0; $p < count($zycode); $p++) {
            $pr = $dicPri->where("drug_code='{$zycode[$p]}'")->where($whpri)->field('price')->select();
            if ($pr[0]['price']) {
                $price[$p] = $pr[0]['price'];
            } else {
                $price[$p] = 0;
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            $data[$i]['drug_units'] = (string)$dw[$i];
            $data[$i]['department'] = session('dpment');
            // if($prilist[$i]['price']>0){
            //   $price = $prilist[$i]['price'];
            // }else{
            //   $price = 0;
            // }
            $data[$i]['price'] = $price[$i];

            $data[$i]['costs'] = $price[$i]*$ypkg[$i];
          }

          for($j = 0; $j < $num; $j++){
              $data[$j]['department'] = $_SESSION['dpment'];
...
//echo $cf_dict->getLastSql();
    ...
    public function sjFuzhi(){
      ...
      $dpment = session('dpment');
      $where['prescription.department'] = $dpment;
      $where['drug_dict.department'] = $dpment;
      $list = $zdict->join('prescription_detail on prescription_detail.presc_no=prescription.presc_no')->join('drug_dict on prescription_detail.drug_code=drug_dict.drug_code')->field('prescription.order_text,prescription.cf_tree,prescription.usage1,prescription.decoction,prescription.dosage,prescription_detail.drug_name,prescription_detail.drug_code,prescription_detail.amount,prescription_detail.drug_units,prescription.presc_name,prescription_detail.usage,drug_dict.xw1')->where($where)->where("prescription_detail.presc_no='$cfh'")->select();
...
//        $where['drug_dict.department'] = session('dpment');
        $where['v.department'] = session('dpment');
        $where['v.flag'] = '1';
        //$dpment = session('dpment');
//        $list = $dict->join('drug_dict on drug_dict.drug_code = bz_cf.ypdm')->where($where)->field('drug_dict.drug_name,drug_dict.xw1,drug_dict.drug_code')->select();
        $list = $dict->join('V_DICT_DRUG_ZY as v on v.code = bz_cf.ypdm')
            ->where($where)
            ->field('v.name, v.xw, v.code')
            ->select();
//        var_dump($list);
        $this->ajaxReturn($list);
    }
    ...
    public function cfDetail(){
      $code = $_POST[code];
      $where['presc_no'] = $code;
      //$where['y_makenature.tree'] = array('like',"0000.002%");
      $where['prescription_detail.department'] = session('dpment');
        $where['V_DICT_DRUG_ZY.department'] = session('dpment');
        $where['V_DICT_DRUG_ZY.flag'] = '1';
      $dict = M('prescription_detail');
      $dict2 = M('prescription');
      $we['department'] = session('dpment');
        $list = $dict->where($where)
            ->join('V_DICT_DRUG_ZY on V_DICT_DRUG_ZY.code=prescription_detail.drug_code')
            ->field('V_DICT_DRUG_ZY.xw,prescription_detail.drug_name,prescription_detail.drug_no,prescription_detail.drug_code as code,prescription_detail.price,prescription_detail.drug_units,prescription_detail.usage,prescription_detail.amount')
            ->order('drug_no')
            ->select();
//      echo $dict->getLastSql();
        $res = $dict2->where("presc_no='$code'")->where($we)->field('order_text,dose,zy_name,bz,zz')->select();
      $list[0]['yz'] = $res[0]['order_text'];
      $list[0]['does'] = $res[0]['dose'];
      $list[0]['zy_name'] = $res[0]['zy_name'];
      $list[0]['bz'] = $res[0]['bz'];
      $list[0]['zz'] = $res[0]['zz'];
//       dump($list);die;
      $this->ajaxReturn($list);
...
//        }
         ...
         */
        $dict = M('tcd_szjj');
        $szjj[0] = $dict->where("CF_TREE='{$code}'")->select();
        if (count($szjj[0]) < 1) {
            $szjj[0] = 999;
        }
    ...
    public function ys()
    {
        ...
        foreach ($xydrug as $key => $val) {
            $xyInf[$key]['yp_name'] = $val['drug_name'];
            //药品名称
            $xyInf[$key]['num'] = floatval($val['yp_total_amount']);
            //数量
            $xyInf[$key]['dw'] = substr(trim($val['drug_spec']), -3);
            //单位
            $xyInf[$key]['js'] = 1;
            //剂数
            $xyInf[$key]['price'] = floatval($val['price']);
            //单价
            $xyInf[$key]['allPrice'] = $val['price'] * floatval($val['yp_total_amount']);
            //总金额
        }
    ...
    {
        session('cz', 1);
        //健康档案处置判断取值位置（1处方0bqxx）
        $xy = M("drug_dict");
        $bianmas = I("post.cfids");
        $shuliangs = I("post.shuliang");
        $bianmaArr = explode(',', $bianmas);
        //药品编码数组
        $shuliangArr = explode(',', $shuliangs);
        //药品数量数组
        $num = count($bianmaArr) - 1;
        //循环次数
        $xyInf = array();
        for ($i = 0; $i < $num; $i++) {
            ...
            //组合成新数组
            $xyInf[$i]['yp_name'] = $xyDrug['drug_name'];
            //药品名称
            $xyInf[$i]['num'] = floatval($shuliangArr[$i]);
            //数量
            $xyInf[$i]['dw'] = substr(trim($xyDrug['drug_spec']), -3);
            //单位
            $xyInf[$i]['js'] = 1;
            //剂数
            $xyInf[$i]['price'] = floatval($xyDrug['price']);
            //单价
            $xyInf[$i]['allPrice'] = $xyDrug['price'] * floatval($shuliangArr[$i]);
            //总金额
        }
        $this->ajaxReturn($xyInf);
    }
    //更改处方的状态为提交审核
    public function upCfind()
    {
        $code = $_POST['code'];
        $dict = M('prescription');
        $data['indicate'] = 50;
