...
/**
 * 配伍禁忌十九畏
 * Class TabooShijiuController
 * @package Admin\Controller
 */
class TabooShijiuController extends AdminController
{
    public function index(){
        ...
        if($ypName){
            //条目数
            $shibaCount=M()->field("t2.drug_name as yuan,t3.drug_name as fan")->table("jy t1,drug_dict t2,drug_dict t3")->where("(t1.department=".session('did')." or t1.department=0) and t2.department='1' and t3.department='1' and t1.YP1=t2.drug_code and t1.YP2=t3.drug_code and LX='2' and (t2.drug_name='".$ypName."' or t3.drug_name='".$ypName."')")->count();
            //页数
            $page = new Page($shibaCount, C('PAGE_SIZE'));
            //十九畏表
            $shiba=M()->field("t2.drug_name as yuan,t3.drug_name as fan,ID")->table("jy t1,drug_dict t2,drug_dict t3")->where("(t1.department=".session('did')." or t1.department=0) and t2.department='1' and t3.department='1' and t1.YP1=t2.drug_code and t1.YP2=t3.drug_code and LX='2' and (t2.drug_name='".$ypName."' or t3.drug_name='".$ypName."')")->order('ID')->limit($page->firstRow, $page->listRows)->select();
        }else{
            //条目数
            $shibaCount=$jy->where("(department=".session('did')." or department=0) and lx='2'")->count();
            //页数
            $page = new Page($shibaCount, C('PAGE_SIZE'));
            //十九畏表
            $shiba=M()->field("t2.drug_name as yuan,t3.drug_name as fan,ID")->table("jy t1,drug_dict t2,drug_dict t3")->where("(t1.department=".session('did')." or t1.department=0) and t2.department='1' and t3.department='1' and t1.YP1=t2.drug_code and t1.YP2=t3.drug_code and LX='2'")->order('ID')->limit($page->firstRow, $page->listRows)->select();
        }
    ...
    public function edit(){
        $id=I("get.id");
        $res=M()->field("t2.drug_name as yuan,t3.drug_name as fan,ID")->table("jy t1,drug_dict t2,drug_dict t3")->where("t1.YP1=t2.drug_code and t1.YP2=t3.drug_code and LX='2' and ID='".$id."'")->find();
